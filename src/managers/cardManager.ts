...
/**
 * Interface for card creation options
 * Allows customization of new card properties
 */
interface CardCreationOptions {
  /** Title for the new card */
  title?: string;
  /** Description for the new card */
  description?: string;
  /** Priority level for the new card */
  priority?: CardPriority;
  /** Tags for the new card */
  tags?: string[];
  /** Assignee for the new card */
  assignee?: string;
  /** Due date for the new card */
  dueDate?: Date;
  /** Target column ID for the new card */
  columnId?: string;
  /** Board ID where the card should be created */
  boardId?: string;
}

/**
 * Interface for card movement operations
 * Defines the parameters for moving cards between columns
 */
interface CardMoveOperation {
  ...
  boardId: string;
  /** New position index within the target column */
  targetIndex?: number;
}
...
export class CardManager {
   ...
   */
  async addCard(options?: CardCreationOptions): Promise<KanbanCard> {
    // Get target board and column
    const { board, columnId } = await this.getTargetBoardAndColumn(
      options?.boardId,
      options?.columnId
    );
    ...
    vscode.window.showInformationMessage(
      `Card "${cardTitle}" added to "${targetColumn.title}"`
    );
  ...
  async moveCard(cardId?: string, targetColumnId?: string): Promise<void> {
    ...
    if (sourceColumn.id === targetColId) {
      vscode.window.showInformationMessage(
        `Card "${card.title}" is already in column "${targetColumn.title}"`
      );
      return;
    }

    // Perform the move operation
    const moveOperation: CardMoveOperation = {
      cardId: targetCardId,
    ...
    this.logger.info(
      `Moved card: ${card.title} from ${sourceColumn.title} to ${targetColumn.title}`
    );
  ...
  async editCard(cardId: string): Promise<void> {
    ...
    if (hasChanges) {
      ...
      vscode.window.showInformationMessage(
        `Card "${card.title}" updated successfully`
      );
    }
  }
   ...
   */
  private async executeCardMove(operation: CardMoveOperation): Promise<void> {
   ...
   */
  private async findCardLocation(cardId: string): Promise<{
    board: KanbanBoard;
    column: any;
    card: KanbanCard;
    cardIndex: number;
   ...
   */
  private async getTargetBoardAndColumn(
    boardId?: string,
    columnId?: string
  ): Promise<{ board: KanbanBoard | undefined; columnId: string | undefined }> {
    let targetBoard: KanbanBoard | undefined;
    let targetColumnId: string | undefined = columnId;
  ...
  private async promptForCardTitle(defaultValue?: string): Promise<string | undefined> {
  ...
  private async promptForCardDescription(defaultValue?: string): Promise<string> {
  ...
  private async promptForCardPriority(defaultValue?: CardPriority): Promise<CardPriority> {
    const priorityOptions = [
      { label: 'Low Priority', value: CardPriority.LOW },
      { label: 'Medium Priority', value: CardPriority.MEDIUM },
      { label: 'High Priority', value: CardPriority.HIGH },
      { label: 'Urgent', value: CardPriority.URGENT }
    ];
    ...
    return selected?.value || defaultValue || CardPriority.MEDIUM;
  }

  private async promptForCardTags(defaultValue?: string[]): Promise<string[]> {
  ...
  private async promptForCardAssignee(defaultValue?: string): Promise<string | undefined> {
    return await vscode.window.showInputBox({
  ...
  private async promptForCardDueDate(defaultValue?: Date): Promise<Date | undefined> {
  ...
  private async promptForColumnSelection(
    board: KanbanBoard,
  ...
  private async promptForCardSelection(): Promise<{ cardId: string; boardId: string } | undefined> {
    const boards = await this.boardManager.getAllBoards();
    const allCards: Array<{ card: KanbanCard; board: KanbanBoard; column: any }> = [];
